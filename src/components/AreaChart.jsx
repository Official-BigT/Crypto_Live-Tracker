...
const AreaChart = ({ historicalData, currencySymbol }) => {
  // Initializes chart data with just header rows
  // Chart expects 2 columns: Date and Price
  const [data, setData] = useState([["Date", "Prices"]]);
  console.log("Price : ", historicalData.prices);
  /* Runs when historical data changes, convert timestamps to JS Date Objects and creates chart-friendly data format*/

  // Add heading to the data array(top of the array)
  ...
  const options = {
    ...
    trendlines: {
      // Configuration for a trendline (a linear regression line)
      0: {
        // Applies to the first (and only) series
        type: "linear",
    ...
    crosshair: {
      // Configuration for the crosshair that appears on hover
