...
const CoinPage = () => {
  ...
  // chartData- stores price, market cap, and volume data for the coin
  const [chartData, setChartData] = useState(null);
  // console.log(chartData);

  // Period- tracks the selected time period for the chart
  const [period, setPeriod] = useState("14");

  // error- tracks any API errors
  const [error, setError] = useState(null);

  // Access currentCurrency from crypto context to display prices in the selected currency
  const { currentCurrency } = useContext(CryptoContext);
  if (!cryptoId) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-800 text-white">
        <p> Error: No crypto ID provided </p>
      </div>
    );
  }
    ...
    const fetchData = async () => {
      try {
        ...
        const chartRes = await fetch(
          `https://api.coingecko.com/api/v3/coins/${cryptoId}/market_chart?vs_currency=${currentCurrency.name}&days=14&interval-=daily`,
          requestOptions
        );
        if (!chartRes.ok)
        ...
        <div className="flex-1 bg-gray-800/30 backdrop-blur-md p-4 rounded-lg md:rounded-xl border border-emerald-500/20">
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                {typeof coinDetails.market_data?.price_change_percentage_24h ===
                "number"
